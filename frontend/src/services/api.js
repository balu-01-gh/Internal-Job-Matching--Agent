...
// Intercept responses to handle both successful responses and errors
api.interceptors.response.use(
  (response) => {
    // For successful responses, ensure data is properly parsed if it's a string
    if (typeof response.data === 'string') {
      try {
        // Attempt to parse, but don't fail if it's not JSON
  ...
  (err) => {
    // For errors, redirect to login on 401 (but NOT for the login endpoint itself)
...
export const authAPI = {
  login: (emailOrUsername, password, hr_id) =>
    api.post('/employee/login', { email: emailOrUsername, password, hr_id }),
  signup: (data) =>
...
export const registerAPI = {
  ...
  teamLookup: (code, hrId) => api.get(`/register/team-lookup/${code}`, { params: { hr_id: hrId } }),
}
